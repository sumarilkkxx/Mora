...
export async function POST(req: NextRequest) {
  ...
  if (files.length > MAX_FILES) {
    return apiError(req, "每次最多上传 5 张图片", "You can upload up to five images at a time");
  }
  ...
  // Validate the full batch before touching disk. This avoids partial uploads when
  // a later file is invalid and lets the bounded write phase run concurrently.
