...
describe("buildComposeInvocation（shell-free 执行形态：修 Windows 合成必挂 issue #13）", () => {
  const cfg: ComposeConfig = {
    ...
    clips: [
      ...
      { type: "image", filePath: "C:\\mora\\data\\s0.png", duration: 3, transition: "direct_concat", motion: "static" },
    ],
  ...
  it("输入路径作为独立 argv token 原样传入（不加引号、不做反斜杠转义）", () => {
    ...
    expect(inv.inputArgs).toContain("C:\\mora\\data\\s0.png");
