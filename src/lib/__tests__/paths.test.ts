...
describe("paths 运行时路径解析", () => {
  ...
  it("注入 APP_DATA_DIR 时所有可写路径都迁过去（Electron 打包关键）", () => {
    ...
    expect(getUploadsDir()).toBe(join("/tmp/daihuo-userdata", "uploads"));
    expect(getOutputDir()).toBe(join("/tmp/daihuo-userdata", "output"));
  });
...
describe("fileNameOf", () => {
  it("extracts the file name from a Windows absolute path", () => {
    expect(fileNameOf("D:\\mora\\data\\output\\proj-1\\final_123.mp4")).toBe("final_123.mp4");
  });
  ...
  it("handles mixed separators (forward-slash Windows paths)", () => {
    expect(fileNameOf("D:/mora\\data/output\\final.mp4")).toBe("final.mp4");
  });
