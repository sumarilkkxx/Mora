...
describe("buildAssetRows", () => {
  ...
  it("本地商品流程的旧 AI 分镜 → 回退到商品原图，不显示丢失素材", () => {
    const rows = buildAssetRows(
      [shot({ shotId: 1, visualSource: "ai_generate" })],
      [],
      ["/uploads/prod.jpg"],
      "local",
    );
    expect(rows[0].visualSource).toBe("product_image");
    expect(rows[0].status).toBe("done");
    expect(rows[0].thumbnailUrl).toBe("/uploads/prod.jpg");
  });

  it("本地成片保留 AI/图库素材，因为素材来源不决定最终渲染方式", () => {
    const rows: SavedAssetRow[] = [
      { shotId: 1, filePath: "/stock/wrong.jpg", status: "done", type: "stock_footage" },
      { shotId: 2, filePath: "/ai/wrong.jpg", status: "done", type: "ai_generated" },
      { shotId: 3, filePath: "/product/right.jpg", status: "done", type: "product_image" },
      { shotId: 4, filePath: "/upload/right.jpg", status: "done", type: "user_upload" },
    ];
    expect([...readyAssetsByShot(rows).keys()]).toEqual([1, 2, 3, 4]);
  });
